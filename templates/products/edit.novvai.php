    ...
    <div class="content-fluid">
        ...
        <div class="col-md-12">
            ...
            <div class="card card-warning">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form role="form" method="POST" action="/products/<?=$product->id?>/edit" enctype="multipart/form-data">
                    <div class="card-body">
                        <img src="<?= $product->thumbnail ?>" class="img img-thumbnail">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input type="number" class="form-control" min="0" step=".01" id="price" name="price" value="<?= $product->price ?>">
                                <?php renderErr(session()->get('errors.price')) ?>
                            </div>
                            <div class="form-group col-md-4">
                                <label for="quantity">Наличност</label>
                                <input type="number" step="1" min='0' class="form-control" id="quantity" name="quantity" value="<?= $product->quantity ?>">
                                ...
                                <?php foreach ($categories as $category) { ?>
                                    <optgroup label="<?= $category->name ?>">
                                        <?php foreach ($category->subCategories() ?? [] as $sub_cat) { ?>
                                            <option value="<?= $sub_cat->id ?>" <?= ($category->category_id == $sub_cat->id) ? "selected='selected'" : "" ?>><?= $sub_cat->name ?></option>
                                        <?php } ?>
                                    </optgroup>
                                <?php } ?>
                            </select>
                        ...
                        <div class="form-group">
                            <div class="custom-control custom-switch">
                                <input type="checkbox" <?=$product->is_featured?"checked='checked'":""?> class="custom-control-input" id="is_featured" name="is_featured">
                            ...
                            <?php foreach ($product->details() as $index => $detail) { ?>
                                <div class="form-row additiona_info">
                                    <div class="form-group  col-sm-3">
                                        <input type="text" class="form-control " placeholder='Обем' name="additional[<?= $index ?>][name]" id="" value="<?= $detail->name ?>">
                                    </div>
                                    <div class="form-group col-sm-3">
                                        <input type="text" class="form-control " placeholder='200 мл.' name="additional[<?= $index ?>][value]" value="<?= $detail->value ?>">
                                    </div>
                                    <div class="form-group col-sm-3">
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        $('[data-nv-click="add_detail_input"]').on('click', (e) => {
            e.preventDefault();
            const randomIndex = randHash(6);
            const container = document.createElement('span');
            container.innerHTML = `<div class="form-row additiona_info">
            <div class="form-group  col-sm-3">
                <input type="text" class="form-control " placeholder='Обем' name="additional[${randomIndex}][name]" id="">
            </div>
            <div class="form-group col-sm-3">
                <input type="text" class="form-control " placeholder='200 мл.' name="additional[${randomIndex}][value]" id="">
            </div>
            <div class="form-group col-sm-3">
                <a href="#" data-nv-action="remove" class="btn btn-danger"><span class="fas fa-times"></span></a>
            </div>
